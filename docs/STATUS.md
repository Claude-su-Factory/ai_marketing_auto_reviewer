...
- 2026-04-21 feat: Plan C 완료 — Winner DB + Voyage RAG. `core/rag/` 모듈(types/db/store/voyage/qualifier/retriever) 도입, `data/creatives.db` SQLite(WAL) 스키마, voyage-3-lite 512d embedding BLOB 저장, 3-stage RAG cascade(category-cosine → global-cosine → lexical Jaccard)로 `generateCopy` few-shot 실데이터 주입. improvementCycle을 aggregate→qualify→runCycle 3단계 DI로 리팩터. 테스트 262 통과.

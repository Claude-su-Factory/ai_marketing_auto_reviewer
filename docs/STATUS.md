...
- **Product 데이터 풍부화 review-deferred items** (2026-04-28, Phase 1 commits `d58ba8b`+`f117bb4`+`8a0ef65`): 3 필드 추가 (learningOutcomes/differentiators/originalPrice) + parser/copy prompt/RAG 임베딩 통합 + 신규 banned-pattern 가드. review 사이클의 deferred 항목.
  ...
  - **R-F4** Google 모델 deprecation 대응 — 2026-04-28 사용자 보고: `imagen-3.0-generate-002` 가 404 NOT_FOUND. 해결: (1) `npm run list-models` 신규 스크립트로 가용 모델 list, (2) `[ai.google.models]` config 섹션 추가 (image/video override), (3) `callGoogleModel` 헬퍼가 404 시 friendly 에러 + list-models 안내, (4) default image 모델 `imagen-3.0-generate-002` → `imagen-4.0-generate-001` 갱신. 미래 deprecation 시 사용자가 list-models 실행 후 config 갱신.

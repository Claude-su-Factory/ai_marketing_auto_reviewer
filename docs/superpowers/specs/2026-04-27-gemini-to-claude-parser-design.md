...
5. 신규 `packages/core/src/creative/geminiRetry.ts`:
   ...
   - maxRetries=3, baseDelayMs=2000 (linear-by-attempt: 2s, 4s — maxRetries=3 ⇒ 2 sleeps, 총 ~6s wait)
...
- baseDelayMs = 2000, delay = baseDelayMs × attempt (linear-by-attempt). maxRetries=3 ⇒ 2 sleeps만 발생: 1번째 fail 후 2s wait, 2번째 fail 후 4s wait, 3번째 fail 즉시 throw. 총 wait ~6s. exponential 아님 — Gemini 안정성 시나리오에 thundering-herd 보호 불필요로 판단

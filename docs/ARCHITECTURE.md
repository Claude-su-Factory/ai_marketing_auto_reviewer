...
| `cli/client/` | ~~Usage Server HTTP 클라이언트 + AI 프록시 추상화~~ **삭제됨 (2026-04-22)** |
| `cli/mode.ts` | ~~Owner/Customer 모드 감지~~ **삭제됨 (2026-04-22)** |
...
**결정**: 자기학습 루프는 TUI/터미널 수명과 독립된 프로세스로 실행하며, Owner 모드(개인 Mac)는 launchd LaunchAgent, Customer 모드(API 서버)는 `server/index.ts` 기동 훅으로 동일 `core/scheduler/` 모듈을 재사용한다. Customer 모드 서버 경로는 2026-04-22 기준 비활성 (server/ 미실행).
...
**How:** `cli/mode.ts`, `cli/client/aiProxy.ts`, `cli/client/usageServer.ts` 삭제. CLI 는 항상 owner 경로(`.env` AI 키 직접 호출)로만 동작. `server/` 코드는 보존하되 실행하지 않는다 (non-active). 커밋: `d3d575e`, `0c626f2`.

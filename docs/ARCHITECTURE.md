...
│ - Review (Ink TUI)                   │◄──────►│ - 레이트 리밋 (10 req/min)   │
│ - Launch (Meta Ads)                  │        │ - Stripe 결제 + Webhook      │
│ - Monitor (성과 수집)                │        │ - SQLite (data/licenses.db)  │
│ - Improve (자율 개선)                │        └──────────────────────────────┘
│                                      │
│ Owner 모드: AI 키 직접 호출          │
│ (Customer 모드: 비활성 —             │
...
| `packages/core/src/creative/` | 카피(Claude), 이미지(Imagen) 생성 로직 (영상 트랙 2026-04-28 제거) |
...
### 6. 영상 트랙 제거 (2026-04-28)

**Why:** Veo 영상 품질 한계 (조잡 / 인위적 / 텍스트 깨짐) + 강의/B2B 광고에 영상 효과 미미 + Meta DCO image-only 정상 지원 → AI 영상 미도입 + 영상 데이터 자체 미취급으로 결정.

**How:** `Creative.videoLocalPath` / `VariantGroup.assets.video` / `AssetFeedSpec.videos` 필드 제거 + Veo 관련 코드 (creative/video.ts, server/jobs/videoJob.ts, server/routes/aiVideo.ts) 삭제. Generate = image + copy 2-track. 자세한 의사결정은 `docs/superpowers/specs/2026-04-28-remove-video-track-design.md` 참조.

### 7. Claude 모델 use-case 별 tier (2026-04-28)

**Why:** 하드코딩된 단일 모델은 비용/품질 트레이드오프를 callsite 별로 표현 못함. Parser (HTML→JSON, mechanical) 는 Haiku 로 충분 (~73% 비용 절감 / scrape), Copy/Analysis/Improver 는 Sonnet 의 한국어 nuance + reasoning 필요.

**How:** `packages/core/src/config/claudeModels.ts` 가 4 use-case 별 상수 (`MODEL_PARSER`/`MODEL_COPY`/`MODEL_ANALYSIS`/`MODEL_IMPROVER`) 를 export. 6 callsites 가 import 후 사용. 명명을 use-case 별로 (HIGH/LOW tier 아님) — 미래 한 사이트만 모델 변경 시 1 줄 수정 가능. 자세한 트레이드오프는 `docs/superpowers/specs/2026-04-28-claude-model-tier-design.md` §3.2 참조.
...
| `data/creatives/` | 생성된 소재 (카피 JSON + 이미지 파일 경로) |

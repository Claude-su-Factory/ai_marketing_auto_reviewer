...
export function buildAdSetTargeting() {
  ...
  const targeting: {
    ...
    publisher_platforms,
    // NOTE: video_feeds (FB) 와 reels (IG) 는 video-native placement — image-only 자산으로
    // 게재 시 delivery underperform 가능 (R-G1 STATUS). 첫 launch 후 placement breakdown 확인.
    // 만약 video_feeds/reels impression 0 이면 image-friendly 만 좁히기:
    //   facebook_positions=[feed, story, marketplace], instagram_positions=[stream, story]
